...
3. Runs `npx changeset version --snapshot <tag>` to generate a snapshot version (e.g. `0.2.0-beta-20260316120000`).
...
npx trmnl-cli@next
npx trmnl-cli@canary
```

## Prerequisites

- The `NPM_TOKEN` secret must be configured in the repository settings with publish access to the `trmnl-cli` package on npm.
